...
if ( ! class_exists( 'C4WP_Captcha_Class' ) ) {
	...
	class C4WP_Captcha_Class {
		...
		public static function enqueue_captcha_scripts() {
			...
			$number         = ( function_exists( 'is_buddypress' ) && is_buddypress() && 'v2_invisble' !== $captcha_version ) ? 1 : $number;
			...
			$lang       = self::determine_captcha_language();
			$google_url = false;
			...
			if ( method_exists( $method, 'get_provider_script_url' ) ) {
				...
				wp_enqueue_script( 'c4wp-method-provider', $google_url, array( 'jquery' ), C4WP_PLUGIN_VERSION, $loading_args );
				wp_enqueue_script( 'c4wp-method', C4WP_PLUGIN_URL . 'includes/methods/js/c4wp-' . $captcha_version . '.js', array( 'c4wp-method-provider' ), C4WP_PLUGIN_VERSION, $loading_args );
				...
				wp_localize_script(
					...
					array(
						...
						'additional_js'       => apply_filters( 'c4wp_captcha_callback_additonal_js', '' ),
						...
						'redirect'            => $redirect,
						'failure_v2_site_key' => $failure_v2_site_key,
					)
				);
			}
		}
		 ...
		 *
		 * @param string $return_markup - Orignal markup.
		 *
		...
		public static function ms_form_field_verify( $result ) {
			if ( isset( $_POST['stage'] ) && 'validate-user-signup' === $_POST['stage'] && ! self::verify() ) {
				...
				if ( class_exists( 'C4WP\\Geo_Blocking' ) ) {
					if ( 'default' != C4WP_Functions::c4wp_get_option( 'comment_rule_countries_method', 'default' ) ) {
						...
						if ( $fails_additional_verify ) {
							...
							} elseif ( 'deny_to_moderation' == C4WP_Functions::c4wp_get_option( 'comment_rule_countries_method', 'default' ) ) {
								$approved = 0;
							} else {
								$approved = 'spam';
							}
